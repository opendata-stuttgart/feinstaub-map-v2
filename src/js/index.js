...
window.onload = function () {
  ...
  async function retrieveData() {
    ...
    await api
      ...
      // await api2
      // .getData("https://maps.sensor.community/data/v2/data.dust.min.json")
      // .then(function (result) {
      //   console.log(result);
      //   sensorsLocations = result;
      // })
...
onEachFeature: function (feature, layer) {
  ...
  function switchWindLayer() {
    if (document.querySelector("#cb_wind").checked) {
      wind.getData(
        config.data_host + "/data/v1/wind.json",
        map,
        switchWindLayer
      );
      document
        .querySelectorAll(".velocity-overlay")
        .forEach((d) => (d.style.visibility = "visible"));
    } else {
      document
        .querySelectorAll(".velocity-overlay")
  ...
  function switchLegend(val) {
    console.log("#legend_" + val);
    document
      .querySelectorAll("[id^=legend_]")
      .forEach((d) => (d.style.display = "none"));
    document.querySelector("#legend_" + val).style.display = "block";
    // if (val == "NO2") {
    //   document.querySelector("#no2_list").style.display = "block";
    // }else{
    //   document.querySelector("#no2_list").style.display = "none";
...
function countDistance(){
  ...
  sensorsInBounds.forEach(function(i){
    if (i._latlng.distanceTo(e._latlng)<=250){e.count250 +=1};
