      ...
      .then((json) => {
        ...
        if (vizType === "pmDefault") {
          let cells = _.chain(json)
            .filter(
              (sensor) =>
                typeof api.pm_sensors[sensor.sensor.sensor_type.name] !=
                  "undefined" &&
                api.pm_sensors[sensor.sensor.sensor_type.name] &&
                api.checkValues(
                  parseInt(getRightValue(sensor.sensordatavalues, "P1")),
                  "PM10"
                ) &&
                api.checkValues(
                  parseInt(getRightValue(sensor.sensordatavalues, "P2")),
                  "PM25"
                )
            )
            .map((values) => {
          ...
          let cells = _.chain(json)
            .filter(
              (sensor) =>
                typeof api.thp_sensors[sensor.sensor.sensor_type.name] !=
                  "undefined" && api.thp_sensors[sensor.sensor.sensor_type.name]
            )
            .map((values) => {
              if (values.timestamp > timestamp_data) {
                timestamp_data = values.timestamp;
              ...
              return {
                data: {
                  ...
                  Humidity: parseInt(
                    getRightValue(values.sensordatavalues, "humidity")
                  ),
                  Temperature: parseInt(
                    getRightValue(values.sensordatavalues, "temperature")
                  ),
                },
                id: values.sensor.id,
          ...
          let cells = _.chain(json)
            .filter(
              (sensor) =>
                typeof api.noise_sensors[sensor.sensor.sensor_type.name] !=
                  "undefined" &&
            ...
            .map((values) => {
              ...
              return {
                data: {
                  Noise: parseInt(
                    getRightValue(values.sensordatavalues, "noise_LAeq")
                  ),
                },
                id: values.sensor.id,
                latitude: values.location.latitude,
